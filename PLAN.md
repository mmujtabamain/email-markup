# ELL — first standalone compiler, CLI, language server, and editor client

## Context

ELL currently exists inside `growth-console`: a Python implementation under
...
is a makeshift prototype and read-only correctness oracle, not a released ELL 1.
...
```text
.ell source + one JSON object -> ellc -> complete HTML
```

The CLI receives the actual data for the render. ELL resolves every `@{…}`
...
```bash
printf '%s' "$LEAD_JSON" | ellc compile message.ell --data-stdin -o message.html
ellc compile message.ell --data-file lead.json -o message.html
```
...
For the initial product integration, the caller invokes `ellc` with the JSON for
...
Only `.ell` source files participate in this release. There is no `.ellt` file,
...
`@Include("file.ell");` composes ELL files. Resolution checks, in order:
...
`${ELL_LIB}` and `${EMAIL_MARKUP_BRAND}` are compiler-defined path variables available
...
allowed root: the project root, an explicit `-I` directory, `${ELL_LIB}`, or
...
and `@Template`, stored in `lib/builtins.ell`. Their email-hardened markup moves
out of C++ and into ELL.

Component prop types remain the existing ELL types. Wrong types, missing
...
`@Template` inside a component definition is the component's static ELL body;
it is expanded completely by `ellc`. It is not the deferred external-engine
templating functionality postponed to `templates.md`.

`brand/example/brand.ell` owns palette and company tokens,
`brand/example/styles.ell` owns style bundles, and
`brand/example/shell.ell` owns the complete email wrapper. The compiler performs
...
ELL source normalisation ports the prototype's `_normalise` and `_trim`
...
The implementation inside `growth-console` is not ELL 1. Until the standalone
...
├── run.sh / run.bat
├── packages/
│   ├── ell-core/                language, renderer, CSS and lint library
│   ├── ell-cli/                 ellc
│   ├── ell-lsp/                 ell-lsp
│   └── ell-tests/               Catch2 suites and golden fixtures
├── lib/
│   └── builtins.ell             fifteen built-in components in ELL
├── brand/example/                brand.ell, styles.ell, shell.ell
├── examples/                    final-HTML `.ell` examples plus JSON fixtures
├── extensions/vscode/           thin LSP client and secure HTML preview
├── grammar/ell.ebnf             normative grammar and recovery boundaries
...
├── ELL.md                       implemented language reference
└── README.md
```

There is intentionally no `lib/engines/` or `.ellt` editor contribution in this
release.
...
**Checkpoint:** `./setup.sh && ./run.sh build` produces stub `ellc`, `ell-lsp`,
and test binaries.

---

## Phase 1 — `packages/ell-core`
...
| `lexer.hpp` | ELL sigils, comments, strings, source ranges |
...
same API. A later `growth-console` adoption can use a binding or `ellc --json`
...
The project configuration identifies one `shell.ell`. Shell lint permits its
...
## Phase 2 — built-ins, brand, shell, and examples in ELL

Create `lib/builtins.ell` from the fifteen Python renderers using
...
**Checkpoint:** `ellc` can compile `examples/solution_first.ell` with its JSON
fixture into complete branded HTML, and shell lint validates unsubscribe and
media requirements.

---

## Phase 3 — `packages/ell-cli` (`ellc`)

```text
ellc build [dir]
ellc compile <file> -o <file>
ellc check <file>
ellc lint <file> [--role content|shell]
ellc fmt <file> [--write]
ellc schema
ellc --version

  -I <dir>            add an allowed include/search directory
  --import <file>     pre-import a `.ell` definition file
...
`ell.json` contains only current-release concepts:
...
`ellc`, `ell-lsp`, `lib/`, brand assets, schema, documentation, and licenses.
...
## Phase 5 — `packages/ell-lsp`

Target the current LSP 3.18 specification over stdio and link `ell-core`
...
- Each workspace folder discovers the nearest ancestor `ell.json` without
...
- Project context contains JSON fixture metadata, imports, shell, and allowed
  include roots. It contains no engine or `.ellt` state.
...
document symbols, folding, signature help, and formatting shared with `ellc
...
`ell/preview` is requested only while a preview panel is visible. It returns the
...
Build one `.ell` language contribution and a thin `vscode-languageclient` host
for the bundled `ell-lsp`. TextMate grammar and language configuration are
...
- Profile edit-to-diagnostics and `ellc build` before changing allocation or
  parsing strategies.
...
- Add an optional tree-aware minifier only if Gmail-clipping evidence warrants
  it.
- Rewrite `ELL.md` and README examples to match the implemented JSON-to-final-HTML
  compiler exactly.
...
10. A negative conformance gate proves `.ellt`, `@Engine`, `@[…]`, and
...
- `.ellt` files, `@Engine`, square-bracket macro calls, and engine-specific
