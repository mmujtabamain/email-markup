...
:mode_ok
if defined ELLC goto :compiler_ready
echo ==^> Step 1/2 - Build the ELL compiler (%BUILD_MODE%)
call "%REPO_DIR%\run.bat" build %BUILD_MODE% || exit /b 1
set "ELLC=%REPO_DIR%\build\%BUILD_MODE%\bin\ellc.exe"
goto :compile

:compiler_ready
echo ==^> Step 1/2 - Use the configured ELL compiler
"%ELLC%" --version || exit /b 1
...
for %%D in (01-interpolation 02-conditionals 03-loops 04-expressions 05-typed-props 06-named-slots 07-tokens 08-includes 09-css-inlining 10-responsive-media) do (
  echo ^> "%ELLC%" compile "%SCRIPT_DIR%%%D\message.ell" -o "%SCRIPT_DIR%%%D\message.html"
  "%ELLC%" compile "%SCRIPT_DIR%%%D\message.ell" -o "%SCRIPT_DIR%%%D\message.html" || exit /b 1
)
