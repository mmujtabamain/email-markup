# Email Markup 1 language reference

Email Markup (EM) is a UTF-8, typed, component-based language for producing final HTML email.
Its complete execution contract is:

```text
.em source + one JSON object -> compiler -> complete HTML
```

Expressions, loops, conditions, components, tokens, styles, includes, and the
shell are resolved by the compiler. Output contains no Email Markup directives.

## Lexical forms

Email Markup recognizes `@` only in these forms:
...
The installed standard library defines fifteen components in ordinary Email Markup:
...
`email-markup-lsp` speaks LSP over stdio with UTF-16 positions and incremental document
...
The VS Code grammar is HTML-first: Email Markup scopes are injected into HTML and CSS, and
...
completion. The Email Markup language server remains the semantic source of truth.
...
Email Markup 1 supports only `.em`. `.emt`, `@Engine`, `@[...]`, and `@Name[...]` are
