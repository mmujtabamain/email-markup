...
class EmbeddedDocuments implements vscode.TextDocumentContentProvider {
  ...
  uri(document: vscode.TextDocument, language: "html" | "css"): vscode.Uri {
    ...
    this.contents.set(key, language === "html" ? htmlProjection(source) : cssProjection(source).text);
    return uri;
  }

  async open(document: vscode.TextDocument, language: "html" | "css"): Promise<vscode.TextDocument> {
    const virtual = await vscode.workspace.openTextDocument(this.uri(document, language));
    if (!this.activated.has(language)) {
      const builtin = vscode.extensions.getExtension(`vscode.${language}-language-features`);
...
async function forwardedCompletions(
  ...
  return token.isCancellationRequested ? new vscode.CompletionList() : (result ?? new vscode.CompletionList());
}
...
export function registerWebFeatures(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.languages.registerCompletionItemProvider(selector, {
      async provideCompletionItems(document, position, token, _completionContext) {
        const source = document.getText();
        const language = webCompletionLanguage(source, document.offsetAt(position));
        if (!language) return new vscode.CompletionList();
        const forwarded = await forwardedCompletions(documents, document, position, language, token);
        if (language !== "html" || !isClassAttributeContext(source, document.offsetAt(position))) {
          return forwarded;
        }
    ...
    vscode.languages.registerHoverProvider(selector, {
      async provideHover(document, position, token) {
        ...
        return token.isCancellationRequested || !hovers?.length ? undefined : hovers[0];
      },
    }),
    ...
    vscode.languages.registerLinkedEditingRangeProvider(selector, {
      provideLinkedEditingRanges(document, position) {
        const ranges = htmlLinkedRanges(document.getText(), document.offsetAt(position));
        return ranges.length === 2 ? {
          ranges: ranges.map(([start, end]) => new vscode.Range(document.positionAt(start), document.positionAt(end))),
    ...
    vscode.languages.registerColorProvider(selector, {
      ...
      async provideColorPresentations(color, colorContext, token) {
        ...
        const presentations = await vscode.commands.executeCommand<vscode.ColorPresentation[]>(
          ...
          color,
          { uri: virtual.uri, range: colorContext.range },
        );
