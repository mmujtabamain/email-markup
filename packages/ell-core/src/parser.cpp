...
std::string trim(std::string value) {
    const auto keep = [](const unsigned char ch) { return !std::isspace(ch); };
...
std::vector<Parameter> parameters(const Token& token, std::vector<Diagnostic>& diagnostics) {
    ...
    const auto commit = [&](const std::size_t end) {
        ...
        for (std::size_t i = 0; i < part.size(); ++i) {
            if (part[i] == '"' && (i == 0 || part[i - 1] != '\\')) in_string = !in_string;
            if (!in_string && part[i] == '(') ++nested;
            if (!in_string && part[i] == ')') --nested;
            if (!in_string && nested == 0 && part[i] == ':') { colon = i; break; }
        }
        if (colon == std::string::npos) {
            diagnostics.push_back({"ELL0201", Severity::error,
        ...
        if (!std::regex_match(name, name_pattern) || expression.empty()) {
            diagnostics.push_back({"ELL0202", Severity::error,
        ...
        if (std::any_of(result.begin(), result.end(), [&](const auto& item) {
            ...
            diagnostics.push_back({"ELL0203", Severity::error,
...
public:
    ...
    ParseResult run() {
        ...
        while (peek().kind != TokenKind::end) {
            error("ELL0204", "Unexpected closing tag @/" + peek().name + ".", peek().range);
            advance();
        }
...
private:
    ...
    bool close(const std::string& name, const SourceRange opened) {
        ...
        error("ELL0205", "@" + name + " is not closed; expected @/" + name + ".",
              opened);
        while (peek().kind != TokenKind::end) {
            if (peek().kind == TokenKind::close && peek().name == name) {
                advance();
            ...
            if (peek().kind == TokenKind::open || peek().kind == TokenKind::self_closing) break;
            advance();
        }
        return false;
    }
    ...
    std::vector<NodePtr> nodes(const std::unordered_set<std::string>& stops,
        ...
        while (peek().kind != TokenKind::end && peek().kind != TokenKind::close &&
            ...
            if (token.kind == TokenKind::text) {
                ...
                if (text.find_first_not_of(" \t\r\n") != std::string::npos) result.push_back(
            ...
            if (token.kind == TokenKind::invalid) { advance(); continue; }
        ...
        if (!result.empty()) {
            ...
            if (auto* text = std::get_if<TextNode>(&result.back()->value)) {
                text->text.erase(std::find_if(text->text.rbegin(), text->text.rend(),
                    [](unsigned char ch) { return !std::isspace(ch); }).base(), text->text.end());
            }
        }
        return result;
    }
    ...
    NodePtr tag(const bool top_level) {
        ...
        if (block_keywords.contains(token.name) && token.kind == TokenKind::self_closing) {
            error("ELL0210", "@" + token.name + " requires a body.", token.range);
            return {};
        }
        if (token.name == "If") return parse_if(token);
        if (token.name == "For") return parse_for(token);
        if (token.name == "Slot") return parse_slot(token);
        if (token.name == "Include") return parse_include(token, top_level);
        if (token.name == "DefineComponent") { parse_component_definition(token, top_level); return {}; }
        if (token.name == "DefineStyle") { parse_style_definition(token, top_level); return {}; }
        if (token.name == "DefineToken") { parse_token_definition(token, top_level); return {}; }
        if (token.name == "Media") { parse_media(token, top_level); return {}; }
        if (reserved_keywords.contains(token.name)) {
            error("ELL0211", "@" + token.name + " is not valid in this context.", token.range);
            return {};
        }
    ...
    NodePtr parse_if(const Token& token) {
        if (trim(token.parameters).empty()) error("ELL0220", "@If requires a condition.", token.range);
    ...
    NodePtr parse_for(const Token& token) {
        ...
        if (!std::regex_match(token.parameters, match, pattern)) {
            error("ELL0221", "@For requires: name in expression.", token.range);
        }
        auto body = nodes({});
    ...
    NodePtr parse_include(const Token& token, const bool top_level) {
        if (!top_level || token.kind != TokenKind::self_closing) {
            error("ELL0222", "@Include is a top-level void construct.", token.range);
        }
        return std::make_shared<Node>(
    ...
    std::string definition_name(const Token& token) {
        ...
        if (found == args.end()) {
            error("ELL0230", "Definition requires name: \"CapitalizedName\".", token.range);
            return {};
        }
        const auto value = literal_string(found->expression);
        if (!value) {
            error("ELL0231", "Definition name must be a string literal.", found->range);
            return {};
        }
        return *value;
    }
    ...
    std::string raw_body(const std::string& name, const Token& opened) {
        std::string raw;
        if (peek().kind == TokenKind::text) raw = advance().text;
        close(name, opened.range);
        return raw;
    }

    void parse_component_definition(const Token& token, const bool top_level) {
        if (!top_level) error("ELL0232", "@DefineComponent is top-level only.", token.range);
        ...
        if (!std::regex_match(definition.name, component_name) ||
            ...
            error("ELL0233", "Invalid or reserved component name “" + definition.name + "”.",
                  token.range);
        }
        bool saw_template = false;
        while (peek().kind != TokenKind::end &&
               !(peek().kind == TokenKind::close && peek().name == "DefineComponent")) {
            if (peek().kind == TokenKind::text && trim(peek().text).empty()) { advance(); continue; }
            if (peek().kind != TokenKind::open) {
                error("ELL0234", "A component definition contains only @Props, @Slots, and @Template.",
                      peek().range);
                advance();
                continue;
            }
            ...
            if (inner.name == "Props") {
                ...
                error("ELL0235", "@" + inner.name + " is not valid in a component definition.",
                      inner.range);
                nodes({});
        ...
        if (!saw_template) error("ELL0236", "Component “" + definition.name +
                                 "” requires @Template.", token.range);
        if (!definition.name.empty()) document_.components[definition.name] = std::move(definition);
    }

    void parse_style_definition(const Token& token, const bool top_level) {
        if (!top_level) error("ELL0240", "@DefineStyle is top-level only.", token.range);
        ...
        if (structural.find('{') != std::string::npos ||
            ...
            error("ELL0241", "A style body contains declarations, not selectors or braces.",
                  token.range);
        }
        if (!name.empty()) document_.styles[name] = {name, body, token.range};
    }

    void parse_token_definition(const Token& token, const bool top_level) {
        if (!top_level || token.kind != TokenKind::self_closing) {
            error("ELL0242", "@DefineToken is a top-level void construct.", token.range);
        }
        const auto args = parameters(token, diagnostics_);
        std::string name;
        std::string value;
        for (const auto& arg : args) {
            if (arg.name == "name") {
                if (auto literal = literal_string(arg.expression)) name = *literal;
            } else if (arg.name == "value") value = arg.expression;
        }
        if (name.empty() || value.empty()) {
            error("ELL0243", "@DefineToken requires string name and value arguments.", token.range);
        } else document_.tokens[name] = {name, value, token.range};
    }

    void parse_media(const Token& token, const bool top_level) {
        if (!top_level) error("ELL0250", "@Media is top-level only.", token.range);
        const auto query = literal_string(trim(token.parameters));
        if (!query) error("ELL0251", "@Media requires a quoted media query.", token.range);
