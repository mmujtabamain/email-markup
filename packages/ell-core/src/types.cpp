...
std::vector<PropDeclaration> parse_prop_declarations(
    ...
    while (std::getline(stream, line)) {
        ...
        if (std::any_of(output.begin(), output.end(), [&](const auto& prop) {
            ...
            error(diagnostics, "ELL0402", "Prop “" + match[1].str() +
                      "” is declared more than once.", line_range);
            continue;
        }
        ...
        if (match[4].matched) {
            ...
            if (declaration.minimum > declaration.maximum) {
                error(diagnostics, "ELL0403", "Prop range minimum exceeds its maximum.",
                      line_range);
            }
        }
        if (match[6].matched) {
...
std::vector<SlotDeclaration> parse_slot_declarations(
    ...
    while (std::getline(stream, line)) {
        ...
        if (std::any_of(output.begin(), output.end(), [&](const auto& slot) {
            ...
            error(diagnostics, "ELL0411", "Slot “" + match[1].str() +
                      "” is declared more than once.", line_range);
            continue;
        }
...
bool validate_prop(const PropDeclaration& declaration, const Json& value,
    ...
    bool type_ok = false;
    if (value.is_null()) type_ok = declaration.optional;
    else if (declaration.type == "string" || declaration.type == "url" ||
             declaration.type == "email" || declaration.type == "color") {
        type_ok = value.is_string();
    } else if (declaration.type == "int") type_ok = value.is_number_integer();
    else if (declaration.type == "number") type_ok = value.is_number();
    else if (declaration.type == "bool") type_ok = value.is_boolean();
    if (!type_ok) {
        error(diagnostics, "ELL0420", "Prop “" + declaration.name + "” requires " +
                  declaration.type + "; values are never coerced.", value_range);
        return false;
    }
    ...
    if (declaration.type == "url") {
        ...
        if (!std::regex_match(raw, url)) {
            error(diagnostics, "ELL0421", "Prop “" + declaration.name +
                      "” is not a URL with a scheme or root-relative path.", value_range);
            return false;
        }
    }
    if (declaration.type == "email") {
        ...
        if (!std::regex_match(value.get_ref<const std::string&>(), email)) {
            error(diagnostics, "ELL0422", "Prop “" + declaration.name +
                      "” is not an email address.", value_range);
            return false;
        }
    }
    if (declaration.has_range) {
        ...
        if (measured < declaration.minimum || measured > declaration.maximum) {
            error(diagnostics, "ELL0423", "Prop “" + declaration.name +
                      "” is outside its declared range.", value_range);
            return false;
        }
    }
    if (!declaration.comparison.empty()) {
        ...
        if (!valid) {
            error(diagnostics, "ELL0424", "Prop “" + declaration.name +
                      "” violates its declared numeric bound.", value_range);
            return false;
        }
    }
    return true;
