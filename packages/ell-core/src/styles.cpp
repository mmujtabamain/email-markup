...
std::string interpolate(std::string text, const EvaluationContext& context,
    ...
    for (; current != end; ++current) {
        ...
        if (evaluated.ok) {
            try { output += emit_scalar(evaluated.value); }
            catch (...) {
                diagnostics.push_back({"ELL0501", Severity::error,
...
CssDeclarations resolve_style_layers(
    ...
    while (names >> name) {
        ...
        if (found == styles.end()) {
            diagnostics.push_back({"ELL0502", Severity::error,
        ...
        const auto parsed = parse_declarations(
            interpolate(found->second.declarations, context, found->second.range,
                        diagnostics));
        for (const auto& [property, value] : parsed) result[property] = value;
    }
    const auto call = parse_declarations(interpolate(std::string{call_css}, context,
                                                     range, diagnostics));
    for (const auto& [property, value] : call) result[property] = value;
    return result;
}
...
std::vector<Diagnostic> validate_media(const MediaDefinition& media) {
    ...
    if (braces != 0) {
        diagnostics.push_back({"ELL0510", Severity::error,
                               "Unbalanced @Media braces; Gmail would discard the block.",
                               media.range});
    }
    if (media.query.empty()) {
        diagnostics.push_back({"ELL0511", Severity::error,
