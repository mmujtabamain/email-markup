...
std::size_t delimited(std::string_view text, std::size_t start, const char opener,
    ...
    while (position < text.size()) {
        ...
        if (quoted) {
            ...
            escaped = ch == '\\' && !escaped;
            if (ch != '\\') escaped = false;
            if (depth > 0) output.push_back(ch);
            continue;
        }
        if (ch == '"') quoted = true;
    ...
    while (position < text.size()) {
        ...
        if (next == '/') {
            if (position + 2 < text.size() && text[position + 2] == '/') {
                position = text.find('\n', position + 3);
                if (position == std::string_view::npos) position = text.size();
                continue;
            }
            flush(start);
            position += 2;
            if (position >= text.size() || !name_start(text[position])) {
                add_error(result, {source, start, position}, "ELL0101",
        ...
        if (next == '{') {
            ...
            if (end == std::string_view::npos) {
                add_error(result, {source, start, text.size()}, "ELL0102",
                          "Unclosed expression; expected }.", diagnostic_limit);
                position = text.size();
            } else {
                if (expression.find_first_not_of(" \t\r\n") == std::string::npos) {
                    add_error(result, {source, start, end}, "ELL0103",
                              "An interpolation expression cannot be empty.", diagnostic_limit);
                }
                result.tokens.push_back({TokenKind::expression, {}, {},
                                         std::move(expression), {source, start, end}});
                position = end;
            }
            continue;
        }
        if (next == '*') {
            ...
            if (end == std::string_view::npos) {
                add_error(result, {source, start, text.size()}, "ELL0104",
        ...
        if (position < text.size() && text[position] == '(') {
            ...
            if (end == std::string_view::npos) {
                add_error(result, {source, start, text.size()}, "ELL0105",
        ...
        if (position < text.size() && text[position] == ';') {
            ++position;
            result.tokens.push_back({TokenKind::self_closing, name, std::move(parameters), {},
                                     {source, start, position}});
            continue;
        }
        result.tokens.push_back({TokenKind::open, name, std::move(parameters), {},
                                 {source, start, position}});
        if (raw_body_names.contains(name)) {
            const auto needle = "@/" + name;
            const auto end = text.find(needle, position);
            if (end == std::string_view::npos) {
                add_error(result, {source, start, text.size()}, "ELL0106",
            ...
            if (end > position) {
                result.tokens.push_back({TokenKind::text, {}, {},
                                         std::string{text.substr(position, end - position)},
                                         {source, position, end}});
            }
            result.tokens.push_back({TokenKind::close, name, {}, {},
                                     {source, end, end + needle.size()}});
