...
struct Options {
    ...
    bool json{};
    bool write{};
    ell::LintRole lint_role{ell::LintRole::content};
};
...
AssetPaths locate_assets(const std::filesystem::path& executable) {
    ...
    AssetPaths paths{binary / "lib", binary / "brand/example", binary / "schema/ell.schema.json"};
    ...
    if (!std::filesystem::exists(paths.library) && std::filesystem::exists(installed / "lib")) {
        ...
        paths.schema = installed / "schema/ell.schema.json";
    }
    if (const char* value = std::getenv("ELL_LIB")) paths.library = value;
...
std::optional<std::filesystem::path> find_config(std::filesystem::path from) {
    ...
    while (!from.empty()) {
        const auto candidate = from / "ell.json";
...
std::filesystem::path expand_path(const std::string& raw,
    ...
    auto expanded = replace_all(raw, "${ELL_LIB}", assets.library.string());
...
ProjectConfig load_config(const std::filesystem::path& input,
    ...
    if (const auto found = find_config(input)) {
        ...
        if (!json.is_object()) throw std::runtime_error("ell.json must contain an object");
    ...
    config.path = std::filesystem::absolute(input).parent_path() / "ell.json";
    config.includes = {assets.library, assets.brand};
    config.imports = {assets.library / "builtins.ell", assets.brand / "brand.ell",
                      assets.brand / "styles.ell"};
    config.shell = assets.brand / "shell.ell";
...
void usage() {
    fmt::print(stderr,
        "ELL compiler {}\n\n"
        "Usage:\n"
        "  ellc build [dir] [options]\n"
        "  ellc compile <file> -o <file> [options]\n"
        "  ellc check <file> [options]\n"
        "  ellc lint <file> [--role content|shell] [options]\n"
        "  ellc fmt <file> [--write]\n"
        "  ellc schema\n"
        "  ellc --version\n\n"
        "Options:\n"
        "  -I <dir>            Add an include and allowed-root directory\n"
        "  --import <file>     Pre-import an ELL definition file\n"
...
Options parse_options(const int argc, const char* const argv[]) {
    ...
    while (index < argc) {
        ...
        if (argument == "-o") options.output = value();
            ...
            if (role == "content") options.lint_role = ell::LintRole::content;
            else if (role == "shell") options.lint_role = ell::LintRole::shell;
...
ell::Json load_data(const Options& options, const ProjectConfig& config) {
    ...
    if (!raw) return ell::Json::object();
    auto result = ell::parse_data(*raw, 1024 * 1024);
    if (!result.ok) throw std::runtime_error(result.diagnostics.front().message);
    return std::move(result.data);
}

ell::CompilationRequest request_for(const std::filesystem::path& input,
...
Json diagnostic_json(const ell::Diagnostic& diagnostic,
                     const ell::CompilationResult& result) {
    Json output{{"code", diagnostic.code},
                {"severity", ell::severity_name(diagnostic.severity)},
...
void print_diagnostics(const ell::CompilationResult& result, const bool json) {
    if (json) {
    ...
    for (const auto& diagnostic : result.diagnostics) {
        ...
        ell::SourcePosition position{};
        const ell::SourceFile* source = nullptr;
        if (result.snapshot && result.snapshot->sources &&
            diagnostic.range.source < result.snapshot->sources->size()) {
        ...
        fmt::print(stderr, "{}:{}:{}: {} {}: {}\n", file, position.line + 1,
                   position.column + 1, ell::severity_name(diagnostic.severity),
...
ell::CompilationResult compile_one(const std::filesystem::path& input,
                                   const Options& options,
                                   const ProjectConfig& config,
                                   const ell::Json& data) {
...
int check_or_lint(const Options& options, const AssetPaths& assets) {
    ...
    if (options.command == "lint" && options.lint_role == ell::LintRole::shell)
        config.shell.reset();
    const auto data = load_data(options, config);
    auto result = compile_one(options.input, options, config, data);
    if (options.command == "lint" && options.lint_role == ell::LintRole::shell && result.ok()) {
        auto findings = ell::lint_html(result.generated.html, ell::LintRole::shell,
...
int format_command(const Options& options) {
    ...
    const auto formatted = ell::format_source(source);
...
int build_command(const Options& options, const AssetPaths& assets) {
    ...
    for (std::filesystem::recursive_directory_iterator it{root, error}, end;
        ...
        if (!it->is_regular_file() || it->path().extension() != ".ell") continue;
