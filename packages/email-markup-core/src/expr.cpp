...
namespace email_markup
{
    namespace
    {
        ...
        private:
            ...
            {
                diagnostics_.push_back({"EM1200", Severity::error, std::move(message), range_, {}, std::move(json_path)});
            ...
            Json arithmetic(const Json &lhs, const Json &rhs, const std::string &op)
            {
                if (!numeric(lhs) || !numeric(rhs))
                    fail("Arithmetic operands must be numbers.");
                ...
                if (integers && op != "/")
                {
                    ...
                    else if (op == "*")
                    {
                        ...
                        {
                            if (left == -1)
                                overflow = right == std::numeric_limits<std::int64_t>::min();
                            else if (right == -1)
                                overflow = left == std::numeric_limits<std::int64_t>::min();
                            else if (left > 0)
                                overflow = right > 0
            ...
            Json parse_unary(const bool enabled)
            {
                ...
                if (take("-"))
                {
                    ...
                    if (!enabled)
                        return {};
                    if (!numeric(value))
                        fail("Unary minus requires a number.");
            ...
            Json parse_primary(const bool enabled)
            {
                ...
                if (current_.kind == Kind::number)
                {
                    ...
                    {
                        fail("Invalid or overflowing number literal.");
                    }
                }
                if (current_.kind != Kind::identifier)
                    fail("Expected an expression value.");
