...
private:
    ...
    NodePtr parse_if(const Token& token) {
        if (trim(token.parameters).empty()) error("EM0220", "@If requires a condition.", token.range);
    ...
    void parse_component_definition(const Token& token, const bool top_level) {
        if (!top_level) error("EM0232", "@DefineComponent is top-level only.", token.range);
        ...
        if (!saw_template) error("EM0236", "Component “" + definition.name +
    ...
    void parse_style_definition(const Token& token, const bool top_level) {
        if (!top_level) error("EM0240", "@DefineStyle is top-level only.", token.range);
    ...
    void parse_media(const Token& token, const bool top_level) {
        if (!top_level) error("EM0250", "@Media is top-level only.", token.range);
        const auto query = literal_string(trim(token.parameters));
        if (!query) error("EM0251", "@Media requires a quoted media query.", token.range);
