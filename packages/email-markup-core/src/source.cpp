...
namespace email_markup
{
    ...
    const SourceFile &SourceManager::get(const SourceId id) const
    {
        if (id >= files_.size())
        {
            throw std::out_of_range("invalid Email Markup source id");
        }
        return files_[id];
    }
    ...
    bool is_valid_utf8(const std::string_view text) noexcept
    {
        for (std::size_t i = 0; i < text.size();)
        {
            ...
            if (continuation == 1 && byte < 0xc2U)
                return false;
            const auto second = static_cast<unsigned char>(text[i + 1]);
            if (continuation == 2 && byte == 0xe0U && second < 0xa0U)
                return false;
            if (continuation == 2 && byte == 0xedU && second >= 0xa0U)
                return false;
            if (continuation == 3 && byte == 0xf0U && second < 0x90U)
                return false;
            if (continuation == 3 && byte > 0xf4U)
                return false;
            if (continuation == 3 && byte == 0xf4U && second >= 0x90U)
                return false;
            i += continuation + 1;
        }
        return true;
